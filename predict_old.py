...
def process(src_image_dir, save_dir):
    ...
    for image_path in image_paths:
        ...
        if r != 1:  # always resize down, only resize up if training with augmentation
            ...
            img0 = cv2.resize(img, (int(w0 * r), int(h0 * r)), interpolation=interp)
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            gn_lks = torch.tensor(img.shape)[[1, 0, 1, 0, 1, 0, 1, 0]].to(device)  # normalization gain landmarks
            det[:, :4] = scale_coords(img.shape[2:], det[:, :4], img.shape).round()
            # Print results
            for c in det[:, -1].unique():
                n = (det[:, -1] == c).sum()  # detections per class
            det[:, 5:13] = scale_coords_landmarks(img.shape[2:], det[:, 5:13], img.shape).round()
