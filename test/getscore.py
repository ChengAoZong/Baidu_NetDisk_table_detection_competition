...
def getscore(model, dataset_path, device):
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if len(det) == len(true_det):
                ...
                for i in range(det.size()[0]):
                    ...
                    # bbox返回值为IOU
                    iou = bbox_iou(torch.tensor(true_box_xywh[col_ind[i]]),torch.tensor(xywh) , x1y1x2y2=False, GIoU=False, DIoU=False, CIoU=False, SIoU=False, eps=1e-7)

                    distance_lb_square = (true_point_lb[col_ind[i]][0] - landmarks[0]*w0) * (true_point_lb[col_ind[i]][0] - landmarks[0]*w0) + \
                                         (true_point_lb[col_ind[i]][1] - landmarks[1]*h0) * (true_point_lb[col_ind[i]][1] - landmarks[1]*h0)
                    distance_lt_square = (true_point_lt[col_ind[i]][0] - landmarks[2]*w0) * (true_point_lt[col_ind[i]][0] - landmarks[2]*w0) + \
                                         (true_point_lt[col_ind[i]][1] - landmarks[3]*h0) * (true_point_lt[col_ind[i]][1] - landmarks[3]*h0)
                    distance_rt_square = (true_point_rt[col_ind[i]][0] - landmarks[4]*w0) * (true_point_rt[col_ind[i]][0] - landmarks[4]*w0) + \
                                         (true_point_rt[col_ind[i]][1] - landmarks[5]*h0) * (true_point_rt[col_ind[i]][1] - landmarks[5]*h0)
                    distance_rb_square = (true_point_rb[col_ind[i]][0] - landmarks[6]*w0) * (true_point_rb[col_ind[i]][0] - landmarks[6]*w0) + \
                                         (true_point_rb[col_ind[i]][1] - landmarks[7]*h0) * (true_point_rb[col_ind[i]][1] - landmarks[7]*h0)
                    if distance_lb_square<=400:
                        landmarks_TP = landmarks_TP + 1
                    else:
                        landmarks_FP = landmarks_FP + 1
                    if distance_lt_square<=400:
                        landmarks_TP = landmarks_TP + 1
                    else:
                        landmarks_FP = landmarks_FP + 1
                    if distance_rt_square<=400:
                        landmarks_TP = landmarks_TP + 1
                    else:
                        landmarks_FP = landmarks_FP + 1
                    if distance_rb_square<=400:
                        landmarks_TP = landmarks_TP + 1
                    else:
                        landmarks_FP = landmarks_FP + 1
                    ...
                    img = cv2.resize(orgimg, (1680, 960))
    ...
    final_score = 0.5*(0.5*precision_landmarks + 0.5*precision_bbox)+0.5*(0.5*recall_landmarks + 0.5*recall_bbox)
...
if __name__ == '__main__':
    ...
    dataset_path = "D:\\competion\\detection\\train\\"
