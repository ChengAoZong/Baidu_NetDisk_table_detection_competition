...
def non_max_suppression_landmark(prediction, conf_thres=0.25, iou_thres=0.45,device = 'cpu',classes=None, agnostic=False, multi_label=False, labels=()):
    ...
    t = time.time()
    output = [torch.zeros((0, 14), device=device)] * prediction.shape[0]
    for xi, x in enumerate(prediction):  # image index, image inference
        # Apply constraints
        # x[((x[..., 2:4] < min_wh) | (x[..., 2:4] > max_wh)).any(1), 4] = 0  # width-height
        x = x[xc[xi]]  # confidence
